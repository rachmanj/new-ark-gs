        ...
        <tbody>
          @foreach ($projects as $project)
              <tr>
                ...
                <td class="text-right">
                  {{ $histories->where('project_code', $project)->where('gs_type', 'po_sent')->first() && $plant_budget->where('project_code', $project)->first() ? number_format($histories->where('project_code', $project)->where('gs_type', 'po_sent')->first()->amount / $plant_budget->where('project_code', $project)->sum('amount') * 100, 2) : '' }}
                </td>
              </tr>
          @endforeach
          <tr>
